...
function getFieldPartField(fieldName: string | undefined, report: Report): PolicyReportField | null {
    if (!fieldName) {
        return null;
    }

    const fieldId = generateFieldPartFieldId(fieldName);
    return Object.values(report.fieldList ?? {}).find(field => field.fieldID === fieldId) ?? null;
}
...
function computeFieldPart(part: FormulaPart, context: FormulaContext): string {
    ...
    const reportField = getFieldPartField(name, report);
    if (!reportField) {
    ...
    if (reportField.type === CONST.REPORT_FIELD_TYPES.FORMULA) {
        const maxDepth = 20;
        let currentFormula = reportField.defaultValue;
        let currentFormulaDepth = 0;
        while (reportField.type === CONST.REPORT_FIELD_TYPES.FORMULA && currentFormulaDepth < maxDepth) {
            currentFormulaDepth++;
            const temp = compute(currentFormula, context);
            if (!temp) {
                break;
            }
            currentFormula = temp;
        }
        if (currentFormulaDepth === maxDepth) {
